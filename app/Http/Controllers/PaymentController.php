...
{
    // public function paypal($lang, $subscriber_id, $type = null)
    // {
    //     return view('payment', compact('lang', 'subscriber_id'));
    // }

    public function processCard($lang, $subscriber_id, Request $request)
    {
        // Handle card payment logic (e.g., validate and redirect to PayPal)
        return response()->json(['redirect' => route('payment.paypal', ['lang' => $lang, 'subscriber_id' => $subscriber_id, 'type' => 'card'])]);
    }

    public function success($lang, $subscriber_id)
    {
        return view('success', compact('lang', 'subscriber_id'));
    }
    ...
    // public function success(Request $request, $subscriber_id)
    // {
    ...
    public function cancel($subscriber_id)
    {
        $subscriber = Subscriber::findOrFail($subscriber_id);
        $lang = app()->getLocale();
     ...
     */
    public function showTransferForm($subscriber_id)
    {
        $subscriber = Subscriber::with('game')->findOrFail($subscriber_id);
        $lang = app()->getLocale();
     ...
     */
    public function storeTransfer(Request $request, $subscriber_id)
    {
        $request->validate([
